 ...
 * GitHub OAuth proxy for Sveltia CMS (Ground Pros LLC).
 *
 * Sveltia CMS opens a popup to <worker>/auth, the user authorizes on
 * GitHub, GitHub redirects to <worker>/callback, and the worker returns
 * a page that postMessages the token back to the CMS admin window.
 */

const ALLOWED_ORIGINS = [
  'https://groundprosky.com',
  'https://www.groundprosky.com',
  'https://caseykeown.github.io',
];
...
export default {
  async fetch(request, env) {
    ...
    // ── health check ───────────────────────────────────────────
    if (url.pathname === '/') {
      return new Response('Ground Pros cms-auth worker is running', { status: 200 });
    }
...
<body>
  ...
  <script>
    (function() {
      ...
      var origins = ${JSON.stringify(ALLOWED_ORIGINS)};
      function tryPostMessage() {
        if (window.opener && !window.opener.closed) {
          ...
          document.body.innerHTML =
            ...
            '<p><strong>Close this window</strong>, then return to the editor and refresh.</p>' +
